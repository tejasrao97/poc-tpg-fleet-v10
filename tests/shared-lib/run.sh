...
#      prints the pod's events and logs; --started-ok accepts started pods whose
#      name matches (Vault before init and unseal).
#
# Requires: bash 4, jq.
...
# Vault is Running but not Ready until it is initialized and unsealed: with
# --started-ok its pods count as done once every container has started
{ pod vault-0 Running false | jq '.status.containerStatuses[] += {started: true}'; } | fixture
pw ':' 1 "--started-ok '^vault-[0-9]+\$'"
grep -q 'rc=0 ' "$TMP/out" && grep -q 'vault-0 .*Started' "$TMP/err" \
  && ok "--started-ok: a started, not Ready pod whose name matches is done (shown as Started)" || bad "--started-ok" "$(cat "$TMP/out" "$TMP/err")"
pw ':' 1
grep -q 'rc=2 reason=POD_TIMEOUT' "$TMP/out" && ok "without --started-ok the same pod is waited for" || bad "without --started-ok" "$(cat "$TMP/out")"
pw ':' 1 "--started-ok '^other-[0-9]+\$'"
grep -q 'rc=2 reason=POD_TIMEOUT' "$TMP/out" && ok "--started-ok applies only to the pods whose name matches" || bad "--started-ok other name" "$(cat "$TMP/out")"
