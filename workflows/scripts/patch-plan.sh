...
#                    spec.storageClassName, the Service fields (the exposure values of
#                    a values patch set them); storageSize/walStorageSize not smaller
#   exposure         a values patch may change instance.exposure and the other
#                    exposure values; loadBalancer without allowedSourceRanges is
#                    warning EXPOSURE_UNRESTRICTED, and patch-cluster.sh waits for
#                    the Services to follow
#   values files     no instance.name, instance.postgresVersion, instance.highAvailability,
#                    instance.storageClassName, instance.serviceType (replaced by
#                    instance.exposure), cluster, patches; sizes not smaller;
...
check_postgres_file() {  # check_postgres_file FILE CLUSTER INSTANCE
  ...
  for k in serviceType serviceAnnotations readOnlyServiceType readOnlyServiceAnnotations; do
    yq -e ".spec.${k} == null" "$REPO/$f" >/dev/null \
      || ferr "$f: spec.${k} comes from the exposure values (instance.exposure, serviceAnnotations, readOnlyExposure, readOnlyServiceAnnotations, allowedSourceRanges): set them in a values patch"
  done
...
check_values_file() {  # check_values_file FILE CLUSTER INSTANCE
  local f="$1" k
  check_yaml_map "$f" || return
  for k in .instance.name .instance.postgresVersion .instance.highAvailability .instance.storageClassName .instance.serviceType .cluster .patches; do
...
for c in $(jq -r '.[].name' <<<"$inv"); do
  ...
  if [[ "${#C_ERRS[@]}" -eq 0 ]]; then
    for i in $(jq -r '.instances[]' <<<"$plan"); do
      ...
      exposure_warning_rendered "$c" "$i" "$out"
