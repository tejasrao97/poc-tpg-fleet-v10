...
for i in $(jq -r '.instances[]' <<<"$plan"); do
  ...
  # A values patch may change the exposure (instance.exposure, D64): the
  # Services must follow the spec before the instance counts as patched
  if ! exposure_follow "$i" 300; then
    record "result.${C}.${i}" FAILED EXPOSURE_NOT_APPLIED "$EXPOSURE_DETAIL"; failed=1; continue
  fi
  record "result.${C}.${i}" SUCCEEDED "" "Running, Synced at ${REV:0:12}; services: ${EXPOSURE_DETAIL}; ${refs:-no patch files referenced}"
done
exit "$failed"
