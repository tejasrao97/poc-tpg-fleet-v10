...
rows="$(jq -c --arg mode "$MODE" --argjson inv "$inv" '
  ...
  | ($res | map(.key)) as $have
  | ($d | to_entries | map(select(.key | startswith("precheck."))) | map({key: (.key | ltrimstr("precheck.")), value: (.value | fromjson)}) | from_entries) as $pre
  | $res + [$expected[] | select(. as $e | $have | index($e) | not)
      | if ($pre[.] // {}).status == "BLOCKED"
        then {key: ., value: {status: "NOT_RUN", reason: "BLOCKED", detail: ("pre-check: " + ($pre[.].reason // ""))}}
        else {key: ., value: {status: "NOT_RUN", reason: "", detail: "stopped by an earlier failure or skipped"}} end]
