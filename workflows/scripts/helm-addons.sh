...
vault_server() {
  ...
  # vault-0 is Ready only after init and unseal (35-vault.sh): the release is
  # done when it has started; the agent injector must be Ready
  HR_NO_WAIT=1 HR_STARTED_OK='^vault-[0-9]+$' hr_release vault vault "$VAULT_CHART_VERSION" "$HASHICORP_REPO" vault "$m" "$note"
}
