...
case "$MODE" in
  ...
  create-instance)
    ...
    posint maxParallel "${P_MAX_PARALLEL:-2}"; posint syncTimeoutSeconds "${P_TIMEOUT:-1800}"; posint prTimeoutSeconds "${P_PR_TIMEOUT:-3600}"
    oneof rolloutMode "${P_ROLLOUT_MODE:-canary}" canary batches all
    paths postgresPatchFilePath "${P_POSTGRES_PATCH_FILES:-}" charts/tpg-instance/patches/
    paths valuesPatchFilePath "${P_VALUES_PATCH_FILES:-}" charts/tpg-instance/patches/
    instance_inputs
    ;;
  network-policy)
