...
#   --started-ok REGEX  pods whose name matches REGEX count as done when every
#                       container has started, Ready or not (vault-0 is Ready
#                       only after init and unseal); the table shows them as Started
#
  ...
  jq -r '
    ...
    | [ $p.metadata.name, ($p.status.phase // "Unknown"), "\($r)/\($n)", ($rs | tostring),
        (if $p.metadata.deletionTimestamp then "Terminating"
         ...
         elif $p.status.phase == "Running" and ($cs | length) > 0 and ([$cs[] | select(.started != true)] | length) == 0
           then "Started"
...
pods_watch() {
  local ns="$1" timeout="$2" sel="" kfn="kubectl" label="" done_fn="" wpid="" min=1 allow_empty=0 started_ok=""
  shift 2
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --started-ok) started_ok="$2"; shift 2 ;;
  ...
  while true; do
    ...
    while IFS=$'\t' read -r name phase ready restarts status cont msg; do
      ...
      if [[ -n "$started_ok" && "$status" == "Started" && "$name" =~ $started_ok ]]; then cls=ok; fi
      key="${name}|${status}"
    ...
    if [[ "$running" -eq 0 && "$done_ok" -eq 1 && "$all_ok" -eq 1 ]]; then
      if [[ "$total" -ge "$min" ]] || { [[ "$allow_empty" -eq 1 ]] && [[ "$total" -eq 0 ]]; }; then
        _tpg_note "${ns}: ${total} pod(s) ready${started_ok:+ or started (${started_ok})}${label:+ (${label})}"
        return 0
      fi
    fi
...
#   stops the install as soon as a pod cannot start. HR_NO_WAIT=1: no --wait;
#   the pods are still watched, and HR_STARTED_OK (a pod name regular
#   expression) lists the pods that only need to start, not to be Ready
#   (vault-0 becomes Ready only after init and unseal).
...
hr_release() {
  ...
  if [[ "${HR_NO_WAIT:-0}" == "1" ]]; then
    ...
    # The same pod table as the other releases; pods named in HR_STARTED_OK
    # only have to start (their readiness depends on a later step)
    pods_watch "$ns" "$secs" --kubectl hr_k --label "Helm release ${name}" \
      ${HR_STARTED_OK:+--started-ok "$HR_STARTED_OK"} || wrc=$?
    if [[ "$wrc" -ne 0 ]]; then
      hr_result "$name" FAILED "${POD_WATCH_REASON:-POD_TIMEOUT}: ${POD_WATCH_DETAIL}"
      exit 1
    fi
  else
